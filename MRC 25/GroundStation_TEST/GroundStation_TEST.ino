...
void processTelemetry(String packet) {
  String fields[15];
  int count = splitCSV(packet, fields, 15);
  if (count < 15) {
  ...
  int16_t ax_raw = fields[5].toInt();
  int16_t ay_raw = fields[6].toInt();
  int16_t az_raw = fields[7].toInt();
  int16_t gx_raw = fields[8].toInt();
  int16_t gy_raw = fields[9].toInt();
  int16_t gz_raw = fields[10].toInt();

  float lat = fields[11] == "nan" ? NAN : fields[11].toFloat();
  float lon = fields[12] == "nan" ? NAN : fields[12].toFloat();
  float alt = fields[13] == "nan" ? NAN : fields[13].toFloat();
  int sats  = fields[14].toInt();
